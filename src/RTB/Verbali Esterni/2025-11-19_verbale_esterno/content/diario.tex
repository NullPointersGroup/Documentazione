...
\subsection{Gestione del codice e Versioning}
    ...
    che contiene sempre il codice in produzione$^{G}$, e un branch$^G$ develop, in cui viene integrato il codice pronto per il testing. Ogni nuova funzionalità$^G$ viene sviluppata in 
    un branch$^G$ dedicato (ad esempio feature/nome-funzionalità) e il merge$^{G}$ avviene tramite pull request$^{G}$, che attiva il processo di revisione e verifica$^{G}$.\\
    A differenza del nostro repository$^{G}$, che elimina automaticamente i branch$^G$ feature dopo il merge, il loro non prevede una pulizia automatica; tuttavia non erano contrari a questa pratica e 
...
\subsection{Organizzazione e Pianificazione$^G$ del Progetto}
    \begin{enumerate}[resume*, itemsep=0pt, parsep=1pt, leftmargin=*, label=\arabic*.]
...
\subsection{Tecnologie, Formazione e Infrastruttura}
    ...
    \begin{itemize}[itemsep=0pt, parsep=1pt, label=--]
        ...
        \item \textbf{Back-end}$^{G}$ (Logica e AI): \textbf{Python}$^{G}$, ambiente ideale per lo sviluppo$^G$ di modelli di intelligenza artificiale$^G$ e machine learning;
...
\subsection{Riscontro su Casi d'Uso e Requisiti}
    Per concludere, abbiamo richiesto un riscontro sugli attori$^{G}$ principali e secondari, procedendo poi alla verifica$^G$ e alla valutazione dei vr{\textbf{Casi d’Uso}}$^{G}$, dei vr{\textbf{Requisiti Funzionali}}$^{G}$, dei vr{\textbf{Requisiti Non Funzionali}}$^{G}$ e dei vr{\textbf{Requisiti di Vincolo}}$^{G}$ precedentemente abbozzati dal gruppo in un documento non ufficiale.\\
