...
È attualmente in corso un ampio dibattito sull'argomento. Sicuramente è utilissimo per risparmiare token, però i modelli attuali, essendo stati addestrati su JSON$^G$ o CSV potrebbero avere difficoltà a comprenderlo.  consiglia comunque di rimanere sul JSON.
\subsection{API REST, quale tecnologia lo implementa e suggerimenti a riguardo}
API$^G$ REST$^G$ è il passaggio fra front-end e backend. Una tecnologia consigliata è FastAPI su Python, molto utilizzato al giorno d'oggi.
...
\begin{enumerate}
	...
	\item Il sistema deve esporre interfacce di integrazione documentate tramite API$^G$ REST.
...
\begin{enumerate}
	...
	\item Separazione netta tra frontend, backend$^G$ e motore AI per maggiore manutenibilità.
