...
È attualmente in corso un ampio dibattito sull'argomento. Sicuramente è utilissimo per risparmiare token$^G$, però i modelli attuali, essendo stati addestrati su JSON$^G$ o CSV potrebbero avere difficoltà a comprenderlo.  consiglia comunque di rimanere sul JSON.
\subsection{API REST, quale tecnologia lo implementa e suggerimenti a riguardo}
API$^G$ REST$^G$ è il passaggio fra front-end e backend. Una tecnologia consigliata è FastAPI su Python$^G$, molto utilizzato al giorno d'oggi.
...
\begin{enumerate}
	...
	\item Sistema di throttling$^G$ intelligente per evitare sovraccarichi in input massivi. In questo caso si impedisce al chatbot di rispondere in immediato ma si aspetta.
\end{enumerate}
...
\begin{enumerate}
	\item Utilizzo di un middleware$^G$ per separare ulteriormente la comunicazione tra i componenti oppure attraverso connettori standard come una fonte dati ODBC.
\end{enumerate}