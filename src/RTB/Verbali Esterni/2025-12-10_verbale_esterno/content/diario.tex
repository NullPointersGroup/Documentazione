...
È attualmente in corso un ampio dibattito sull'argomento. Sicuramente è utilissimo per risparmiare token, però i modelli attuali, essendo stati addestrati su JSON o CSV potrebbero avere difficoltà a comprenderlo.  consiglia comunque di rimanere sul JSON.
\subsection{API REST, quale tecnologia lo implementa e suggerimenti a riguardo}
API REST è il passaggio fra front-end e backend. Una tecnologia consigliata è FastAPI su Python, molto utilizzato al giorno d'oggi.
...
Langchain, framework Python per caricare i dati (modello e versione). Tramite questa serie di API c'è la possibilità di passare il contesto su cui noi vogliamo operare. Si passa il dataset.
...
Questo porta direttamente all'autenticazione con il caso d'uso di autenticazione non riuscita.\\
...
È stato discusso lo scenario di invio dati da parte dell'admin per il training, l'esportazione del log degli ordini per analisi esterne, il consulto dello  ordini, sia da parte dell'admin relativamente a tutti i clienti sia da parte del cliente per il suo storico. Questo caso d'uso si soddisfa con una pagina relativa.\\
L'admin può inoltre verificare lo stato dei servizi AI, creare nuovi utenti, cambiare i permessi di questi ultimi.\\
Lo scenario del logout è facilmente comprensivo del caso d'uso del logout dall'utente.\\
Ci è stato detto dall'azienda che visualizzare o terminare le sessioni attive è un caso d'uso che si può tranquillamente trascurare perché non molto sensato.\\
Il caso d'uso di duplicazione ordine è stato confermato, poiché utile per alcuni utenti che ordinano buona parte delle volte gli stessi prodotti.\\
...
L'admin può inoltre scaricare il log della pipeline di un ordine, addestrare manualmente il modello AI, pianificare retraining automatizzato, configurare integrazione ERP, pulire i dati nel training (rimozione di dati rumorosi nel dataset), testare connessione ERP, monitorare utilizzo modelli AI.\\
...
\begin{enumerate}
	...
	\item L'ordine deve essere generato in formato JSON solo dopo la conferma dell'ordine.
	...
	\item Il formato audio deve avere una durata massima di 2 minuti, accettare solo audio con dimensione massima 10MB, supportare la codifica di UTF-8 e supporto dei formati .mp3, .m4a, .wav. Oltretutto dentro la chat deve essere permessa la registrazione per non scomodare l'utente con i limiti mantenuti precedentemente.
...
\begin{enumerate}
	...
	\item Supporto al caricamento di dataset aziendali per test o benchmarking.
	\item Sincronizzazione periodica con ERP per catalogo prodotti e stati ordine.
...
\begin{enumerate}
	\item Supporto multilingua, integrazione con canali aggiuntivi (WhatsApp, Telegram).
	...
	\item OCR obbligatorio se la modalità immagine è attiva.
	\item Modalità simulazione/sandbox senza inserimento in ERP.
	\item Esportazione log/ordini in CSV/JSON per analisi esterne.
	\item Cambio modelli AI (GPT4, GPT5, Gemini ecc).
	\item Connettori plug-in per input da email/chat.
	\item Supporto multi-tenant (dati e cataloghi separati per cliente). Comunque è centrale nel progetto il DB iniziale e non necessario separare i cataloghi.
...
\begin{enumerate}
	...
	\item Garantire la sicurezza dei dati trattati, chiaramente le password non devono essere trasmesse in chiaro. Se tutti i collegamenti fra API vengono gestite con HTTPS la sicurezza è garantita.
...
\begin{enumerate}
	...
	\item Deve supportare backup e ripristino dei dati e delle configurazioni critiche, già fatto da GitHub.
\end{enumerate}
...
\begin{enumerate}
	\item Utilizzo di un database relazionale.
	\item La documentazione deve essere conforme.
	\item Il sistema deve adottare un'architettura modulare.
	\item Il sistema deve esporre interfacce di integrazione tramite API REST documentate.
	\item Il progetto deve utilizzare un sistema di versionamento del codice.
	\item I formati di scambio dati devono essere strutturati in un formato compatibile con il database gestionale.
	\item Utilizzo di un database vettoriale.
...
\begin{enumerate}
	\item Adozione di API REST anche per comunicazione interne tra moduli.
	\item Separazione netta tra frontend, backend e motore AI per maggiore manutenibilità.
	\item Utilizzo di un formato standard per il logging (JSON logging).
\end{enumerate}
