...
\begin{enumerate}\setlength{\itemsep}{0pt}
    \item \textbf{Creazione della Issue}: viene aperta una issue$^G$ su GitHub$^G$, assegnandola a uno o più membri in base al ruolo da loro ricoperto (vedi sezione 4.1.2), per tracciare le attività da svolgere.
    ...
    \item \textbf{Creazione della Pull Request}: una volta completato il lavoro sul branch$^G$, viene aperta una Pull Request$^G$ con richiesta di verifica$^G$ della nuova sezione da un altro membro del gruppo, segnalato nel Registro delle Modifiche.
...
\subsubsubsection{Verifica della Conformità}
	...
	\begin{enumerate}[itemsep=3pt, parsep=1pt, label=\arabic*.]
		...
		\item Il \textbf{Verificatore}, durante la revisione di una Pull Request$^G$, deve controllare:
...
\begin{enumerate}[itemsep=3pt, parsep=1pt, leftmargin=*]
    ...
    \item Quando un membro effettua un push nel branch$^G$ relativo alla issue$^G$, il sistema genera automaticamente una \textbf{pull request}.
...
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
    ...
    \item \textbf{Metriche di Qualità del processo}: indicatori per valutare l'efficacia$^G$ e l'efficienza$^G$ delle attività di sviluppo$^G$.
\end{itemize}
