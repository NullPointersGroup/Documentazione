...
\subsubsection{Caricamento in Repository}
    Ogni volta che si inserisce un nuovo documento o si effettua una modifica$^G$ nel Repository$^{G}$ si utilizza un ramo feature/* personalizzato.\\
    Una volta creato un commit nel branch$^{G}$ una GitHub$^G$ Action$^{G}$ si occupa di creare una pull request$^{G}$ automaticamente la quale deve essere approvata dai Verificatori$^{G}$.
