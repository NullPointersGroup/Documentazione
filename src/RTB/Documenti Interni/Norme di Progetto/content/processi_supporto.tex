...
\subsubsection{Caricamento in Repository}
    Ogni volta che si inserisce un nuovo documento o si effettua una modifica nel Repository$^{G}$ si utilizza un branch feature/* personalizzato.\\
    Una volta creato un commit nel branch$^{G}$ una GitHub Action$^{G}$ si occupa di creare automaticamente una pull request$^{G}$, la quale deve essere approvata dai Verificatori$^{G}$.
