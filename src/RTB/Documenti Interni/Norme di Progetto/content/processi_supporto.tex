...
\subsection{Documentazione}
    Il processo$^G$ di documentazione è un elemento cardine di tutti i processi primari. Il suo output è fondamentale per tracciare le decisioni$^{G}$ prese e per facilitare il lavoro asincrono, che nel nostro contesto si rivela notevolmente più produttivo di quello sincrono. \\
...
\subsubsubsection{Verifica della Conformità}
	...
	\begin{enumerate}[itemsep=3pt, parsep=1pt, label=\arabic*.]
		\item Il \textbf{Programmatore} è responsabile$^G$ della documentazione del proprio codice.
...
Ogni attività, modifica$^G$ o produzione$^G$ di documentazione viene gestita attraverso una issue, che deve contenere:
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
    ...
    \item \textbf{Milestone}: indica l’obiettivo di riferimento (RTB$^G$ o PB);
    \item \textbf{Board e Stato}: identifica la project board di appartenenza e lo stato corrente (Todo, In Progress, Done);
\end{itemize}
Le issue$^G$ vengono create inizialmente dal Responsabile, ma possono essere aggiornate da qualsiasi membro qualora emergano nuove informazioni, modifiche o necessità di riformulazione del compito. Ogni aggiornamento deve mantenere chiarezza, coerenza e tracciabilità.\\
...
\begin{enumerate}[itemsep=3pt, parsep=1pt, leftmargin=*]
    \item \textbf{Creazione della issue} da parte del Responsabile$^G$ o del membro che individua la necessità del compito.
    ...
    \item Quando un membro effettua un push nel branch$^G$ relativo alla issue, il sistema genera automaticamente una \textbf{pull request}.
    \item \textbf{Assegnazione della PR} al Verificatore$^G$ designato.
    \item \textbf{Verifica$^G$ della PR}: il Verificatore$^G$ controlla che il contenuto sia conforme agli standard, coerente con la documentazione e privo di errori.
...
\end{enumerate}
Questa procedura permette tracciabilità completa del lavoro e sincronizzazione costante tra issue, project board e repository$^G$ riducendo il rischio di incoerenze dei documenti.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.37\textwidth}|p{0.61\textwidth}|}
        ...
        \hline
        feature/norme-progetto$^G$ & Per modifiche al documento Norme di Progetto$^G$$^G$ \\
        \hline
        feature/piano-progetto$^G$ & Per modifiche al documento Piano di Progetto$^G$$^G$ \\
        \hline
...
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
    ...
    \item \textbf{MINOR}: Incrementato una volta completato l'intero ciclo di modifica, che include sia lo sviluppo$^G$ del contenuto sia la verifica$^G$ formale da parte del Verificatore$^G$ designato.
...
Il gruppo adotta una pianificazione$^G$ esplicita delle attività di qualità, formalizzata nel Piano di Qualifica, che definisce:
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
...
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
    ...
    \item \textbf{Metriche di Qualità del processo}: indicatori per valutare l'efficacia$^G$ e l'efficienza$^G$ delle attività di sviluppo.
\end{itemize}
...
Per supportare le attività di Verifica$^G$ descritte, il gruppo utilizza GitHub, imponendo che ogni modifica$^G$ alla documentazione o al codice venga prima sottoposta a revisione attraverso una Pull Request.\\ 
