...
\begin{enumerate}[itemsep=2pt, parsep=1pt, leftmargin=*]
    ...
    \item \textbf{Assegnazione della pull request} al Verificatore designato (specificato nella issue).
...
Per migliorare l’organizzazione, il gruppo utilizza un sistema di etichette che identifica chiaramente la tipologia di lavoro. Per ottimizzare la tracciabilità, abbiamo strutturato un sistema di etichette che funziona su due livelli.\\ 
Oltre alle consuete etichette di processo, abbiamo introdotto una famiglia dedicata specificamente alla gestione documentale.\\
...
Queste label vengono applicate sia alle issue settimanali, sia alle attività associate alle milestone RTB e PB. Ciò permette di individuare rapidamente quali attività richiedono attenzione e tracciare il carico di lavoro per tipologia di attività oltre che avere una visione completa dell’avanzamento complessivo del progetto.
...
\subsubsubsection{Ruoli e permessi}
Per garantire l'integrità del processo di Controllo della Configurazione sul repository, sono stabiliti i seguenti ruoli con permessi distinti (Teams di GitHub):
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
    \item \textbf{Responsabile}: Ha il controllo completo del repository.
    \item \textbf{Verificatori}: Sono gli unici membri, oltre al Responsabile, autorizzati a effettuare il merge delle Pull Request nel branch main, previa approvazione del contenuto.
    \item \textbf{Redattori} (Membri del Gruppo): Autorizzati a creare feature branch e a inviare Pull Request per le attività assegnate.
\end{itemize}
...
In base a quanto definito nel \ulhref{https://nullpointersgroup.github.io/Documentazione/output/RTB/Verbali\%20Interni/2025-11-06\_verbale\_interno.pdf}{verbale interno del 6 novembre} per il tracciamento durante il ciclo di vita dei documenti, NullPointers Group ha deciso di utilizzare una politica di versionamento basata sulla specifica MAJOR.MINOR.PATCH. Questa specifica riflette lo stato di avanzamento e la validazione formale del documento. La sua implementazione avviene come segue:
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
...
L'analisi statica è una tecnica di verifica che si applica senza eseguire il codice o il prodotto. Il suo scopo è individuare problemi di sintassi, logica o conformità agli standard prima che si manifestino durante l'esecuzione.\\
