...
Le metriche rappresentano uno strumento essenziale per garantire un monitoraggio continuo e oggettivo della qualità del prodotto software e dei processi impiegati per realizzarlo. Esse permettono di individuare tempestivamente anomalie, inefficienze o deviazioni dagli standard prefissati, favorendo una gestione consapevole dell’evoluzione del progetto.\\
Per garantire chiarezza e coerenza interna, ogni metrica è classificata secondo la seguente notazione:
\begin{itemize}
    \item MQC\_xx - Metriche di qualità del processo
    \item MQD\_xx - Metriche di qualità del prodotto
\end{itemize}

% ============================================================
%              6.2 METRICHE DI PROCESSO
% ============================================================
...
\begin{itemize}
% MQC_01
\item \textbf{MQC\_01 - Earned Value}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
            \text{Earned Value} =
            \text{Planned Value} \times \text{Completeness Issue}
        \]
        \item \textbf{Descrizione}: misura il valore del lavoro effettivamente completato rispetto alla pianificazione. Riflette l’avanzamento reale del progetto in termini economici ed è utile per identificare scostamenti precoci rispetto al piano.
    \end{itemize}

% MQC_02
\item \textbf{MQC\_02 - Planned Value}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
            \text{Planned Value} = \text{Costo preventivato}
        \]
        \item \textbf{Descrizione}: indica il valore del lavoro che avrebbe dovuto essere completato in un dato momento secondo la pianificazione. È utilizzato come riferimento per confrontare l’avanzamento reale con quello previsto.
    \end{itemize}

% MQC_03
\item \textbf{MQC\_03 - Actual Cost}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
            \text{Actual Cost} = \text{Consuntivo}
        \]
        \item \textbf{Descrizione}: rappresenta il costo reale sostenuto fino a una certa data. Confrontato con l’Earned Value consente di valutare l’efficienza economica del progetto.
    \end{itemize}
...
\item \textbf{MQC\_04 - Cost Performance Index}:
    \begin{itemize}
        ...
        Misura l'efficienza economica del progetto confrontando il valore realizzato con il costo effettivamente sostenuto.\\
...
\item \textbf{MQC\_05 - Schedule Performance Index}:
    \begin{itemize}
        ...
        \]
        \item \textbf{Descrizione}: valuta l’efficacia dell’avanzamento temporale. Un valore pari o superiore a 1 indica che il progetto è in linea con la pianificazione o in anticipo rispetto ad essa.
    \end{itemize}

% MQC_06
\item \textbf{MQC\_06 - Estimate at Completion}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
            \text{Estimate at Completion} = 
            \frac{\text{Budget at Completion}}{\text{Cost Performance Index}}
        \]
...
% MQC_08
\item \textbf{MQC\_08 - Time Estimate at Completion}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
            \text{Time Estimate at Completion} =
            \frac{\text{Tempo previsto}}{\text{Schedule Performance Index}}
        \]
        \item \textbf{Descrizione}: indica la data stimata di completamento del progetto sulla base dell’andamento attuale. È utile per valutare possibili ritardi e adottare misure correttive.
    \end{itemize}

% MQC_09
\item \textbf{MQC\_09 - Completeness Issue}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
            \text{Completeness Issue} =
            \frac{\text{Issue chiuse}}{\text{Issue totali}}
        \]
        \item \textbf{Descrizione}: misura il grado di completamento delle attività pianificate, considerando il rapporto tra le issue chiuse e quelle registrate. Un valore elevato indica un buon avanzamento delle attività di sviluppo.
    \end{itemize}
\end{itemize}
...
% MQC_11
\item \textbf{MQC\_11 - Test Success Rate}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
            \text{Test Success Rate} =
            \frac{\text{Test superati}}{\text{Test totali}} \times 100
        \]
        \item \textbf{Descrizione}: indica la percentuale di test automatizzati o manuali che si concludono con esito positivo. Riflette la stabilità delle funzionalità verificate.
    \end{itemize}

% MQC_12
\item \textbf{MQC\_12 - Test Density and Automation}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
            \text{Test Density and Automation} =
            \frac{\text{Test automatizzati}}{\text{Test totali}} \times 100
        \]
        \item \textbf{Descrizione}: misura il grado di automazione del processo di verifica. Un’elevata densità di test automatizzati migliora l’affidabilità della pipeline di testing e riduce i tempi di validazione.
    \end{itemize}
\end{itemize}
...
\item \textbf{MQC\_13 - Quality Metrics Satisfied}:
    \begin{itemize}
        ...
        \]
        \item \textbf{Descrizione}: valuta la percentuale di metriche qualitative che hanno raggiunto il valore accettabile o ottimale. È un indicatore complessivo dell’efficacia del processo di gestione della qualità.
    \end{itemize}
\end{itemize}
...
\item \textbf{MQD\_02 - Requisiti desiderabili soddisfatti}:
    \begin{itemize}
        ...
        \[
            ...
            \right) \times 100
        \]
        \item \textbf{Descrizione}: indica il grado di completamento dei requisiti desiderabili, utili a migliorare l’esperienza d’uso o arricchire il sistema con funzionalità non essenziali.
    \end{itemize}
...
\item \textbf{MQD\_03 - Requisiti opzionali soddisfatti}:
    \begin{itemize}
        ...
        \[
            ...
            \right) \times 100
        \]
        \item \textbf{Descrizione}: valuta l’implementazione di funzionalità opzionali che aggiungono valore al sistema ma non influiscono sulla sua correttezza di base.
    \end{itemize}
\end{itemize}
...
\item \textbf{MQD\_04 - Code Coverage}:
    \begin{itemize}
        ...
        \[
            \text{Code Coverage} =
            \frac{\text{LOC testate}}{\text{LOC totali}} \times 100
        \]
		Dove LOC: Lines of code
        \item \textbf{Descrizione}: misura la percentuale di linee di codice che risultano coperte da test automatici. Una copertura elevata contribuisce a ridurre la probabilità di malfunzionamenti e garantisce maggiore stabilità nelle fasi di manutenzione.
    \end{itemize}

% MQD_05
\item \textbf{MQD\_05 - Branch Coverage}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
            \text{Branch Coverage} =
...
\begin{itemize}
% MQD_07
\item \textbf{MQD\_07 - Time on Task}:
    \begin{itemize}
        \item \textbf{Formula}:
    \[
        \text{Time on Task}_{\text{task}} = \frac{\sum_{i=1}^{n} T_i}{n}
    \]
    dove:\\
...
\begin{itemize}
% MQD_09
\item \textbf{MQD\_09 - Code Smells per KLOC}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
            \text{Code Smells per KLOC} =
            \frac{\text{Numero di code smells}}{\text{KLOC}}
        \]
...
% MQD_11
\item \textbf{MQD\_11 - Cyclomatic Complexity}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
