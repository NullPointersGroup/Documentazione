...
Le metriche qui definite consentono di monitorare l’andamento dei processi primari e di supporto, permettendo di verificare l’aderenza alla pianificazione$^G$, l’efficacia$^G$ delle attività svolte e la qualità complessiva del ciclo di sviluppo.
...
\item \textbf{MQC\_01 - Earned Value$^G$}:
    \begin{itemize}
        ...
        \]
        \item \textbf{Descrizione}: misura il valore del lavoro effettivamente completato rispetto alla pianificazione. Riflette l’avanzamento reale del progetto$^G$ in termini economici ed è utile per identificare scostamenti precoci rispetto al piano.
    \end{itemize}
...
\item \textbf{MQC\_03 - Actual Cost$^G$}:
    \begin{itemize}
        ...
        \]
        \item \textbf{Descrizione}: rappresenta il costo reale sostenuto fino a una certa data. Confrontato con l’Earned Value$^G$ consente di valutare l’efficienza$^G$ economica del progetto.
    \end{itemize}

% MQC_04
\item \textbf{MQC\_04 - Cost Performance Index$^G$}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
            \text{Cost Performance Index$^G$} =
...
% MQC_05
\item \textbf{MQC\_05 - Schedule Performance Index$^G$}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
            \text{Schedule Performance Index$^G$} =
...
\item \textbf{MQC\_06 - Estimate at Completion$^G$}:
    \begin{itemize}
        ...
        \[
            ...
            \frac{\text{Budget at Completion$^G$}}{\text{Cost Performance Index$^G$}}
        \]
...
\item \textbf{MQC\_08 - Time Estimate at Completion$^G$}:
    \begin{itemize}
        ...
        \[
            ...
            \frac{\text{Tempo previsto}}{\text{Schedule Performance Index$^G$}}
        \]
...
\item \textbf{MQC\_09 - Completeness Issue$^G$}:
    \begin{itemize}
        ...
        \]
        \item \textbf{Descrizione}: misura il grado di completamento delle attività pianificate, considerando il rapporto tra le issue$^G$ chiuse e quelle registrate. Un valore elevato indica un buon avanzamento delle attività di sviluppo.
    \end{itemize}
\end{itemize}
...
\item \textbf{MQC\_12 - Test$^G$ Density and Automation}:
    \begin{itemize}
        ...
        \]
        \item \textbf{Descrizione}: misura il grado di automazione$^G$ del processo$^G$ di verifica. Un’elevata densità di test$^G$ automatizzati migliora l’affidabilità della pipeline di testing e riduce i tempi di validazione.
    \end{itemize}
\end{itemize}
...
\item \textbf{MQD\_04 - Code Coverage}:
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: misura la percentuale di linee di codice che risultano coperte da test$^G$ automatici. Una copertura elevata contribuisce a ridurre la probabilità di malfunzionamenti e garantisce maggiore stabilità nelle fasi di manutenzione.
    \end{itemize}
...
\begin{itemize}
% MQD_07
\item \textbf{MQD\_07 - Code Smells per KLOC$^G$}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
...
% MQD_08
\item \textbf{MQD\_08 - Cyclomatic Complexity$^G$}:
    \begin{itemize}
        \item \textbf{Formula}:
        \[
...
%MQD_09
\item \textbf{MQD\_09 - Cognitive Complexity$^G$}:
\begin{itemize}
	\item \textbf{Formula}:
	\[
		\text{Cognitive Complexity$^G$} = \sum \text{(incrementi per annidamento e salti logici)}
	\]
	Dove:
	\begin{itemize}
		\item incremento per ogni struttura di controllo annidata (if, while, for, switch)
		\item incremento per salti logici che interrompono il flusso sequenziale (break, continue, return)
		\item non incrementa per percorsi separati allo stesso livello
	\end{itemize}
	\item \textbf{Descrizione}: misura la complessità del codice dal punto di vista della comprensione umana, valutando quanto è difficile seguire il flusso logico. Valori elevati indicano funzioni difficili da leggere e mantenere.
\end{itemize}
