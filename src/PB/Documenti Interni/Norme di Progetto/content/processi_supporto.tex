...
Di seguito sono elencati tutti i documenti redatti e mantenuti durante l'intero ciclo di vita del progetto$^G$ ciascuno dei quali risponde a uno scopo specifico, contribuendo alla tracciabilità, alla gestione e alla comunicazione delle attività di progetto.\\
...
\subsubsection{Strumenti a supporto}
	...
	\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
	    \item \textbf{LaTeX}: Linguaggio di markup consolidato e ampiamente utilizzato per la stesura di documenti tecnici e scientifici. 
        LaTeX consente di mantenere una elevata qualità tipografica e di gestire in modo efficiente la struttura del documento. NullPointers Group 
        utilizza LaTeX per la produzioneG di tutta la documentazione, facendo uso di pacchetti e template appositamente sviluppati dai membri del gruppo;
	    \item \textbf{Git}: Sistema di versionamento del codice LaTeX;
	    \item \textbf{GitHub}: Per il mantenimento di una repository centrale del codice LaTeX e dei suoi artefatti in formato PDF, che vengono poi resi fruibili
	    attraverso GitHub stesso su un sito web statico. 
	\end{itemize}
...
Come già ribadito, il tracciamento dei requisiti rappresenta un elemento fondamentale per il processo$^G$ di validazione$^G$, permettendo di verificare che ogni requisito obbligatorio sia stato implementato e che il sistema operi correttamente nel suo complesso. 
Per realizzare l’attività di validazione$^G$, il gruppo si avvale del \href{https://nullpointersgroup.github.io/Documentazione/output/RTB/Documenti%20Esterni/Piano_di_Qualifica.pdf}{Piano di Qualifica$^G$} per definire e tracciare le metodologie di test$^G$ implementate.

\subsubsubsection{Attività di validazione}
L’attività di Validazione$^G$ consiste nella verifica$^G$ finale della conformità del prodotto rispetto ai requisiti definiti e concordati con \textbf{Ergon Informatica srl}.\\
Viene svolta attraverso l’analisi degli esiti dei test$^G$ di accettazione e il controllo del tracciamento dei requisiti, costituendo la fase conclusiva del processo$^G$ di verifica$^G$ del sistema. L'obiettivo è accertare che le funzionalità$^G$ realizzate rispondano effettivamente alle aspettative della proponente.
