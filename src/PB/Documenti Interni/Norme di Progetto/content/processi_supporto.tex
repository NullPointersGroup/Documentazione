...
\subsection{Documentazione}
    Il processo$^G$ di documentazione è essenziale per tracciare le decisioni e favorire il lavoro asincrono, nel nostro contesto più produttivo di quello sincrono.\\
    Registra le informazioni di ogni attività del ciclo di vita del prodotto, gestendo tutte le operazioni sui documenti: pianificazione$^G$, progettazione$^G$, sviluppo$^G$, produzione$^G$, modifica$^G$, distribuzione$^G$ e manutenzione.
...
Inoltre la sezione \textbf{Ordine del giorno} precede l'indice delle tabelle e delle immagini e ha lo scopo di delineare in modo strutturato e puntuale gli argomenti che verranno discussi durante la riunione.\\
...
I verbali esterni sono documenti che registrano ufficialmente gli incontri avvenuti con soggetti esterni al gruppo di lavoro, in particolare con l'ente proponente$^G$ del progetto.\\
...
Seguono la struttura base dei documenti esposta alla sezione \ref{sec: struttura-base-doc} ma la sezione \textbf{Ordine del giorno} precede l'indice delle tabelle e delle immagini e ha lo scopo di delineare in modo strutturato e puntuale gli argomenti che verranno discussi durante la riunione.\\
Inoltre diversamente dal resto della documentazione non contengono la sezione del Registro delle Modifiche.\\
Il contenuto principale è composto da:
...
Di seguito sono elencati tutti i documenti redatti e mantenuti durante l'intero ciclo di vita del progetto$^G$, la struttura iniziale di tali documenti corrisponde con quanto definito al punto 3.1.4:
    \begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
        ...
        \item \textbf{Specifica Tecnica$^G$};
        \item \textbf{Manuale Utente$^G$}.
    \end{itemize}
...
\begin{enumerate}\setlength{\itemsep}{0pt}
    ...
    \item \textbf{Revisione e Approvazione}: il membro del team designato procede ad esaminare il codice e il contenuto della PR$^G$, fornendo feedback$^G$ e richiedendo modifiche se necessario.
...
(https$^G$://nullpointersgroup.github.io/Documentazione/) dove, sempre per convenzione, è stato stabilito come segue:
\begin{center}
    AAAA-MM-GG\_TIPO v\#
\end{center}
...
\subsubsubsection{Verifica della conformità}
	...
	\begin{enumerate}[itemsep=3pt, parsep=1pt, label=\arabic*.]
		...
		\end{itemize}
		\item La mancata conformità è motivo di richiesta di modifica$^G$ prima dell'approvazione della PR.
	\end{enumerate}

\subsubsection{Strumenti a supporto}
	Per la stesura e la gestione dei documenti il gruppo utilizza i seguenti strumenti: \vspace{-0.5em}
	\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
	    \item \textbf{LaTeX$^G$}: Linguaggio di markup consolidato e ampiamente utilizzato per la stesura di documenti tecnici e scientifici. Consente di mantenere una elevata qualità tipografica e di gestire in modo efficiente la struttura del documento. NullPointers Group utilizza LaTeX$^G$ per la produzioneG di tutta la documentazione;
	    \item \textbf{Git$^G$}: Sistema di versionamento$^G$ del codice LaTeX$^G$;
	    \item \textbf{GitHub$^G$}: Per il mantenimento delle repository. 
	\end{itemize}

\subsection{Gestione della Configurazione}
\label{sec:gestione-configurazione}
La gestione delle configurazioni è la pratica che garantisce il controllo e la tracciabilità di tutte le componenti di un progetto$^G$ software. Consente di organizzare le modifiche al codice e alla documentazione, permettendo un controllo su tutte le modifiche e le versioni rilasciate.\\
...
\begin{enumerate}[itemsep=3pt, parsep=1pt, leftmargin=*]
    ...
    \item \textbf{Merge$^G$ nel main}: se la verifica$^G$ ha esito positivo, il Verificatore$^G$ effettua il merge$^G$ e chiude la issue. In caso contrario, richiede ulteriori modifiche.
\end{enumerate}
...
Ogni issue$^G$ viene collegata alla board pertinente e inserita nella colonna appropriata, così da garantire una visione aggiornata e condivisa dello stato del progetto.\\
...
Per migliorare l'organizzazione, il gruppo utilizza un sistema di etichette che identifica la tipologia di lavoro.\\
Viene quindi aggiunta un'etichetta specifica, come \vr{Piano di Progetto$^G$} o \vr{Analisi dei Requisiti$^G$}, che indica precisamente quale file è coinvolto.\\
Queste label vengono applicate sia alle issue$^G$ settimanali, sia alle attività associate alle milestone$^G$ RTB$^G$ e PB.\\
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.37\textwidth}|p{0.61\textwidth}|}
        ...
        \hline
        feature/glossario$^G$ & Per modifiche al documento Glossario$^G$ \\
        \hline
        feature/analisi-requisiti & Per modifiche al documento Analisi dei Requisiti$^G$ \\
        \hline
        feature/specifica-tecnica & Per modifiche al documento Specifica Tecnica$^G$ \\
        \hline
        feature/manuale-utente & Per modifiche al documento Manuale Utente$^G$ \\
        \hline
...
In base a quanto definito nel \href{https://nullpointersgroup.github.io/Documentazione/output/RTB/Verbali%20Interni/2025-11-06_verbale_interno.pdf}{verbale interno del 6 novembre} per il tracciamento durante il ciclo di vita dei documenti, NullPointers Group ha deciso di utilizzare una politica di versionamento$^G$ basata sulla specifica MAJOR.MINOR.PATCH. Questa specifica riflette lo stato di avanzamento e la validazione$^G$ formale del documento. La sua implementazione avviene come segue:
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
    \item \textbf{MAJOR$^G$}: Incrementato solo in seguito all'Approvazione Formale (Approvazione) del file da parte del Responsabile.
...
\subsubsection{Strumenti a supporto}
    Per il controllo e la gestione dello Stato di Configurazione il gruppo utilizza i seguenti strumenti:
	\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
	    \item \textbf{GitHub$^G$ Issue$^G$ Tracking/ Project Board}: Strumenti di tracciamento delle attività da svolgere all' interno del progetto$^G$, consentono una pianificazione$^G$ accurata
        dei ruoli e delle scadenze, monitorando l' avanzamento dei lavori di sviluppo.   
	\end{itemize}

\subsection{Gestione della Qualità}
\label{sec:gestione-qualita}
La Gestione della Qualità è il processo$^G$ che definisce, pianifica e coordina le attività necessarie affinché i prodotti sviluppati rispettino gli standard del Piano di Qualifica$^G$ e siano coerenti con gli obiettivi progettuali.\\
Tale processo$^G$ garantisce che ogni artefatto prodotto soddisfi i criteri qualitativi prefissati e fornisce un quadro operativo per il miglioramento continuo. La qualità è assicurata da un sistema integrato di strumenti, procedure e responsabilità che accompagna l'intero ciclo di sviluppo$^G$, prevenendo errori, rilevando incongruenze e garantendo la verifica$^G$ di ogni output.\\
...
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
    ...
    \item Un cruscotto$^G$ di valutazione che consente il monitoraggio, per la durata del ciclo di vita del progetto$^G$, delle metriche di qualità definite.
\end{itemize}
Il Piano di Qualifica$^G$ viene aggiornato nel corso del progetto$^G$ per l'aggiornamento dei dati e la continuazione del cruscotto$^G$ di valutazione, oltre che per eventuali miglioramenti o variazioni negli strumenti adottati.
...
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
    ...
    \item \textbf{Metriche di Qualità del processo$^G$}: indicatori per valutare l'efficacia$^G$ e l'efficienza$^G$ delle attività di sviluppo.
\end{itemize}
...
Il suo scopo principale è rispondere alla domanda \vr{\textit{Did I build$^G$ the system right?}}, ovvero assicurare che quanto prodotto sia stato realizzato correttamente, in piena conformità con i vincoli, le regole e gli standard qualitativi definiti nel Piano di Qualifica$^G$ e nelle Norme di Progetto.\\ 
...
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
    \item \textbf{Walkthrough$^G$}: è una tecnica di verifica$^G$ basata sull'ipotesi che esista un difetto, pur in assenza di informazioni sulla sua natura o localizzazione. Per questo motivo, richiede un'analisi molto approfondita e completa di tutto ciò che si sta esaminando. Di conseguenza, è un metodo che richiede molte risorse, è difficile da applicare nella pratica e non si può automatizzare facilmente.
    \item \textbf{Ispezione}: una lista di controllo già pronta, concentrandosi su errori tipici o criteri di qualità specifici. Anche se è meno approfondita di un walkthrough$^G$, soprattutto all'inizio, ha un grande vantaggio: è facile da automatizzare. Per questo è la scelta migliore quando bisogna controllare tanto codice o tanti documenti in modo sistematico e ripetitivo.
\end{itemize}

\subsubsection{Analisi dinamica}
L'analisi dinamica verifica$^G$ il software eseguendolo con test$^G$ specifici, per misurarne la qualità funzionale e individuare errori nel codice.\\
Ogni test$^G$ ha uno stato iniziale, input e output attesi. I test$^G$ devono essere ripetibili e automatizzabili, così da poterli eseguire continuamente durante l'intero ciclo di vita del prodotto.

\subsubsection{Classificazione dei Test}
Tutti i test$^G$ pianificati, eseguiti e i loro esiti sono tracciati nel Piano di Qualifica.\\
...
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
    \item \textbf{TU-Classificazione}: Test$^G$ di Unità, dove Classificazione può essere:
    \begin{itemize}
    	\item B: utilizzato per i test$^G$ Backend$^G$
    	\item F: utilizzato per i test$^G$ Frontend$^G$
    \end{itemize}
    \item \textbf{TI}: Test$^G$ di Integrazione;
    \item \textbf{TS}: Test$^G$ di Sistema;
    \item \textbf{TA}: Test$^G$ di Accettazione.
\end{itemize}
Il numero \textbf{(xx)} è un valore progressivo che identifica univocamente il test$^G$ all'interno della sua categoria. \\
Inoltre per permettere un monitoraggio efficace dell'avanzamento ogni test$^G$ può assumere uno dei seguenti stati:
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
...
I Test$^G$ di Unità rappresentano il livello di granularità più basso e mirano a verificare il corretto funzionamento delle singole unità software nella loro forma più elementare (funzioni, metodi o classi).
Si distinguono due approcci principali:
\begin{itemize}
    \item \textbf{Test$^G$ Funzionali}: analizzano input e output del sistema senza considerare la logica interna di implementazione. Per questo motivo, il solo test$^G$ funzionale non è sufficiente a garantire la correttezza delle unità e necessita di essere affiancato da altre tipologie di test$^G$;
    \item \textbf{Test$^G$ Strutturali}: analizzano la logica interna del codice, verificandone i diversi percorsi di esecuzione e la relativa copertura.
\end{itemize}

\subsubsubsection{Test di Integrazione}
I Test$^G$ di Integrazione verificano che le diverse componenti, precedentemente testate singolarmente, interagiscano e comunichino correttamente tra loro.\\
...
I test$^G$ di Sistema verificano il comportamento del sistema nella sua interezza, valutandone la conformità rispetto ai requisiti funzionali e non funzionali definiti nel documento di Analisi dei Requisiti. Questi test$^G$ permettono di validare aspetti quali la correttezza funzionale, l'affidabilità, la robustezza e la gestione degli errori.

\subsubsubsection{Test di Accettazione}
I Test$^G$ di Accettazione verificano che il sistema funzioni nella sua interezza e che soddisfi le aspettative ed i requisiti richiesti dalla proponente.

\subsubsubsection{Test di Regressione}
I Test$^G$ di Regressione hanno lo scopo di verificare che le modifiche apportate al sistema, siano esse correzioni o l'aggiunta di nuove funzionalità$^G$, non abbiano compromesso comportamenti precedentemente testati e corretti. 
Vengono eseguiti ogni volta che viene apportata una modifica$^G$ al software e consistono nella riesecuzione dell'intera suite di test$^G$, garantendo così che le funzionalità$^G$ già verificate mantengano la loro correttezza, prevenendo di fatto una regressione della qualità del prodotto.

\subsubsection{Validazione}
Il processo$^G$ di Validazione$^G$ ha l'obiettivo di verificare e confermare attraverso delle prove oggettive che quanto realizzato rispetti tutte le richieste di \textbf{Ergon Informatica Srl}.
Mentre la verifica$^G$ si concentra sulla conformità tecnica del prodotto, la validazione$^G$ risponde alla domanda: \textit{\vr{Did I build$^G$ the right system?}} ovvero \textit{\vr{Stiamo costruendo il sistema corretto?}}.\\
In conformità allo standard ISO/IEC 122207:1995 adottato, le attività previste dal processo$^G$ di validazione$^G$ sono:
\begin{itemize}
    \item esecuzione dell'attività
    \item attività di validazione$^G$
\end{itemize}

\subsubsubsection{Implementazione del processo}
A seguito dell'analisi delle richieste di \textbf{Ergon Informatica srl}, Nullpointers Group ha definito i requisiti da soddisfare, disponibili nel documento di \href{https://nullpointersgroup.github.io/Documentazione/output/PB/Documenti\%20Esterni/Analisi_dei_Requisiti.pdf}{Analisi dei Requisiti$^G$} (Sezione 2.3).\\
Il tracciamento dei requisiti è fondamentale per la validazione$^G$, poiché permette di verificare che ogni requisito obbligatorio sia implementato e che il sistema funzioni correttamente. Per questa attività, il gruppo utilizza il \href{https://nullpointersgroup.github.io/Documentazione/output/PB/Documenti\%20Esterni/Piano_di_Qualifica.pdf}{Piano di Qualifica$^G$}, dove definisce e traccia le metodologie di test$^G$ adottate.

\subsubsubsection{Attività di validazione}
L'attività di Validazione$^G$ consiste nella verifica$^G$ finale della conformità del prodotto rispetto ai requisiti concordati con \textbf{Ergon Informatica srl}. Viene svolta analizzando gli esiti dei test$^G$ di accettazione e il tracciamento dei requisiti, rappresentando la fase conclusiva del processo$^G$ di verifica$^G$ del sistema.\\
L'obiettivo è accertare che le funzionalità$^G$ realizzate rispondano effettivamente alle aspettative della proponente.

\subsubsection{Strumenti a supporto}
    Per il controllo e la gestione dello Stato di Configurazione il gruppo utilizza i seguenti strumenti:
	\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
	    \item \textbf{GitHub$^G$ Pull Request$^G$}: Strumento nativo di GitHub$^G$ che permette ogni modifica$^G$ a codice o a documentazione venga sottoposta a una revisione da parte di un 
        Verificatore$^G$ diverso dall'autore della modifica$^G$ prima di essere integrata nel ramo principale.
	\end{itemize}
