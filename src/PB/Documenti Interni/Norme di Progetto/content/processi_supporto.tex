...
\subsection{Documentazione}
    Il processo di documentazione è essenziale per tracciare le decisioni e favorire il lavoro asincrono, nel nostro contesto più produttivo di quello sincrono.\\
    Registra le informazioni di ogni attività del ciclo di vita del prodotto, gestendo tutte le operazioni sui documenti: pianificazione, progettazione, sviluppo, produzione, modifica, distribuzione e manutenzione.
...
Di seguito sono elencati tutti i documenti redatti e mantenuti durante l'intero ciclo di vita del progetto, la struttura iniziale di tali documenti corrisponde con quanto definito al punto 3.1.4:
    \begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
...
\subsubsection{Strumenti a supporto}
	...
	\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
	    \item \textbf{LaTeX}: Linguaggio di markup consolidato e ampiamente utilizzato per la stesura di documenti tecnici e scientifici. Consente di mantenere una elevata qualità tipografica e di gestire in modo efficiente la struttura del documento. NullPointers Group utilizza LaTeX per la produzioneG di tutta la documentazione;
	    \item \textbf{Git}: Sistema di versionamento del codice LaTeX;
	    \item \textbf{GitHub}: Per il mantenimento delle repository. 
	\end{itemize}
...
\subsubsection{Strumenti a supporto}
	...
	\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
	    \item \textbf{GitHub Issue Tracking/ Project Board}: Strumenti di tracciamento delle attività da svolgere all' interno del progetto, consentono una pianificazione accurata
        dei ruoli e delle scadenze, monitorando l' avanzamento dei lavori di sviluppo.   
	\end{itemize}

\subsection{Gestione della Qualità}
\label{sec:gestione-qualita}
La Gestione della Qualità è il processo che definisce, pianifica e coordina le attività necessarie affinché i prodotti sviluppati rispettino gli standard del Piano di Qualifica e siano coerenti con gli obiettivi progettuali.\\
Tale processo garantisce che ogni artefatto prodotto soddisfi i criteri qualitativi prefissati e fornisce un quadro operativo per il miglioramento continuo. La qualità è assicurata da un sistema integrato di strumenti, procedure e responsabilità che accompagna l'intero ciclo di sviluppo, prevenendo errori, rilevando incongruenze e garantendo la verifica di ogni output.\\
...
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
    ...
    \item \textbf{Ispezione}: una lista di controllo già pronta, concentrandosi su errori tipici o criteri di qualità specifici. Anche se è meno approfondita di un walkthrough, soprattutto all'inizio, ha un grande vantaggio: è facile da automatizzare. Per questo è la scelta migliore quando bisogna controllare tanto codice o tanti documenti in modo sistematico e ripetitivo.
\end{itemize}

\subsubsection{Analisi dinamica}
L'analisi dinamica verifica il software eseguendolo con test specifici, per misurarne la qualità funzionale e individuare errori nel codice.\\
Ogni test ha uno stato iniziale, input e output attesi. I test devono essere ripetibili e automatizzabili, così da poterli eseguire continuamente durante l'intero ciclo di vita del prodotto.
...
A seguito dell'analisi delle richieste di \textbf{Ergon Informatica srl}, Nullpointers Group ha definito i requisiti da soddisfare, disponibili nel documento di \href{https://nullpointersgroup.github.io/Documentazione/output/RTB/Documenti\%20Esterni/Analisi_dei_Requisiti.pdf}{Analisi dei Requisiti} (Sezione 2.3).\\
Il tracciamento dei requisiti è fondamentale per la validazione, poiché permette di verificare che ogni requisito obbligatorio sia implementato e che il sistema funzioni correttamente. Per questa attività, il gruppo utilizza il \href{https://nullpointersgroup.github.io/Documentazione/output/RTB/Documenti\%20Esterni/Piano_di_Qualifica.pdf}{Piano di Qualifica}, dove definisce e traccia le metodologie di test adottate.

\subsubsubsection{Attività di validazione}
L'attività di Validazione consiste nella verifica finale della conformità del prodotto rispetto ai requisiti concordati con \textbf{Ergon Informatica srl}. Viene svolta analizzando gli esiti dei test di accettazione e il tracciamento dei requisiti, rappresentando la fase conclusiva del processo di verifica del sistema.\\
L'obiettivo è accertare che le funzionalità realizzate rispondano effettivamente alle aspettative della proponente.

\subsubsection{Strumenti a supporto}
    Per il controllo e la gestione dello Stato di Configurazione il gruppo utilizza i seguenti strumenti:
	\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
	    \item \textbf{GitHub Pull Request}: Strumento nativo di GitHub che permette ogni modifica a codice o a documentazione venga sottoposta a una revisione da parte di un 
        Verificatore diverso dall'autore della modifica prima di essere integrata nel ramo principale.
	\end{itemize}
