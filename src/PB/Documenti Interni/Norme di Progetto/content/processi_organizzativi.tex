...
La rotazione dei ruoli nella fase RTB$^G$ avviene ogni due settimane, in modo da garantire una distribuzione$^G$ equilibrata delle competenze e favorire l’apprendimento trasversale all’interno del gruppo.\\
...
Nella fase PB$^G$, il gruppo ha optato per sprint$^G$ di 1 settimana. La chiara definizione dei ruoli e la maggiore efficienza$^G$ nello svolgimento dei task$^G$, maturata durante la fase preparatoria, hanno portato alla necessità di ridurre la durata degli sprint.

\subsubsection{Coordinamento}
Un buon coordinamento è fondamentale per il progetto$^G$: permette di gestire efficacemente il lavoro del team e le relazioni con proponente$^G$ e committenti. Per garantire un flusso informativo continuo e aggiornato, sono previste riunioni regolari e l'uso di canali di comunicazione adeguati.

\subsubsubsection{Riunioni}
Al fine di garantire un flusso di comunicazione efficace, un costante allineamento del gruppo e un confronto attivo con la proponente$^G$, sono previsti due tipi distinti di incontri: riunioni interne e incontri con la proponente.
...
La gestione dell'infrastruttura fornisce, configura e mantiene l'ambiente di lavoro necessario per lo sviluppo$^G$ e la documentazione.\\
Comprende la gestione delle risorse hardware e software, garantendone la disponibilità e l'efficienza$^G$ per tutta la durata del progetto.
\subsubsection{Attività previste}
...
\textbf{Project Board}\\
Le board per le milestone$^G$ (RTB$^G$, PB$^G$, MVP$^G$) vengono create da un template predefinito. Al termine di una milestone$^G$, le board vengono archiviate dopo aver chiuso le issue$^G$ residue.\\[1em]
...
\begin{itemize}[itemsep=3pt, parsep=1pt, label=$\scriptstyle\bullet$]
    \item \textbf{Plan (Pianificazione$^G$)}: si identificano e descrivono i Processi Organizzativi da applicare al ciclo di vita del software.
...
In base allo standard ISO/IEC 12207:1995, è necessario fare una revisione dei requisiti del progetto. Questo serve a capire quali competenze i membri di NullPointers Group dovranno acquisire per completare il progetto$^G$ didattico.\\
