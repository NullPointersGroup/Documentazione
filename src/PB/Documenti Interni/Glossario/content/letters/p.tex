...
\term{PascalCase}
Convenzione di denominazione in cui ogni parola che compone un identificatore inizia con la lettera maiuscola, senza spazi o separatori.\\
...
\term{Path}
Percorso che identifica la posizione di una risorsa all’interno di un file system o di una struttura di directory.
...
\term{POC}
Codice stabilito dal Team, rappresenta un’attività con corrispettiva issue in GitHub all’interno della repository dedicata al \textit{Proof of Concept}. Il codice è seguito da un numero incrementale che permette di distinguere univocamente una singola attività.

\term{PostgreSQL}
Sistema di gestione di database relazionali open source, noto per la sua scalabilità e la conformità agli standard SQL.
...
\term{Prompt}
Testo o input fornito a un sistema, in particolare a un modello di intelligenza artificiale, per guidarne la risposta o il comportamento.
